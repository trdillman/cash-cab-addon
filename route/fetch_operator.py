...
try:
    ...
    _STATE_MANAGER_IMPORT_ERROR = None
except ImportError as exc:
    RouteStateManager = None
    _STATE_MANAGER_IMPORT_ERROR = exc
...
try:
    ...
    route_resolve = route_assets = route_anim = route_nodes = route_preview = route_pipeline_finalizer = route_state_manager = route_utils = None
    print(f"[BLOSM] WARN route submodule import failed: {exc}")
...
class BLOSM_OT_FetchRouteMap(bpy.types.Operator):
    ...
    def _ensure_base_scene(self, context):
        ...
        anim_props = {}
        for prop in [
            'blosm_anim_start', 'blosm_anim_end', 'blosm_lead_frames',
            'blosm_route_start', 'blosm_route_end'
        ]:
            if hasattr(scene, prop):
                anim_props[prop] = getattr(scene, prop)
        ...
        # Restore animation properties to the new scene
        if anim_props:
            for key, value in anim_props.items():
                try:
                    setattr(new_scene, key, value)
                except Exception:
                    pass
    ...
    def _ensure_prepared(self, context, log=True, report_tag='ERROR'):
        ...
        try:
            ...
            # Prefer dynamically resolved user agent (from prefs) with a
            # reasonable fallback to the configured default.
            user_agent = getattr(self, "_resolved_user_agent", None) or self._resolve_user_agent(context)

            # Smart water import: always enabled after toggle removal.
            # This used to be gated by addon.route_import_water.
            include_water = True
            ...
            try:
                ...
                # so that external audits can verify what was actually used.
                try:
                    ...
                    # Do not fail the operator if syncing properties fails
                    pass
    ...
    def _capture_state(self, addon):
        keys = (
            ...
            "relativeToInitialImport", "coordinatesAsFilter"
        )
    ...
    def _ensure_route_curve(self, context, coords):
        ...
        obj.select_set(False)
        # Tag and record the OSM route curve so downstream helpers can resolve it
        try:
            obj["blosm_origin"] = "osm"
            obj["blosm_role"] = "route_curve_osm"
        except Exception:
            pass
        scene = getattr(context, "scene", None)
        addon = getattr(scene, "blosm", None) if scene else None
        if addon is not None:
            try:
                addon.route_curve_obj = obj
                addon.route_curve_name = obj.name
            except Exception:
                pass
        return obj
    ...
    def _ensure_car_asset(self, context):
        ...
        if scene and scene.collection and car_obj and car_obj.name not in scene.collection.objects.keys():
            try:
                ...
                pass
        # Tag the resolved car object for robust downstream resolution
        if car_obj is not None:
            try:
                car_obj["blosm_role"] = "asset_car"
            except Exception:
                pass
        return car_obj
...
class BLOSM_OT_ExtendCityArea(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        self._expand_m = max(0.0, float(getattr(addon, "route_extend_m", 0.0) or 0.0))
        if self._expand_m <= 0:
            self.report({'INFO'}, "Extend amount is zero; nothing to do")
            return {'CANCELLED'}

        south, west, north, east = stored_bbox
        lat_pad = _meters_to_lat_delta(self._expand_m)
        mid_lat = (south + north) * 0.5
        lon_pad = _meters_to_lon_delta(self._expand_m, mid_lat)
        self._new_bbox = (south - lat_pad, west - lon_pad, north + lat_pad, east + lon_pad)

        new_tiles_set = set(_tile_bbox(*self._new_bbox))
        self._delta_tiles = [t for t in new_tiles_set if t not in stored_tiles]
        
        if not self._delta_tiles:
            self.report({'INFO'}, "No new tiles to fetch (already covered). Increase extend distance.")
            return {'CANCELLED'}

        # Calculate stats for dialog
        from . import performance_tracker
        tile_count = len(self._delta_tiles)
        eta_seconds = performance_tracker.calculate_eta(tile_count)
        eta_formatted = performance_tracker.format_eta(eta_seconds)
        
        # Approximate area of new tiles
        total_area = 0.0
        for t in self._delta_tiles:
            w, h = route_utils.bbox_size(t)
            total_area += (w * h)
    ...
    def draw(self, context):
        ...
        stats = getattr(self, "_dialog_stats", None)
        if not stats:
            layout.label(text="Calculation error")
            return

        layout.label(text=f"Extending by: {stats['extend_m']:.0f} m")
        layout.label(text=f"New Tiles: {stats['tile_count']}")
        layout.label(text=f"New Area: {stats['area_m2']:,.0f} m²")
        layout.label(text=f"Estimated Time: {stats['eta_formatted']}")
        layout.label(text="Ready to fetch?", icon='QUESTION')

    def execute(self, context):
        # If invoked directly (no dialog), calculate needed values
        if not self._delta_tiles or not self._new_bbox:
            scene = context.scene
            addon = getattr(scene, "blosm", None)
            if addon is None:
                return {'CANCELLED'}
        ...
        # Main execution logic using calculated _delta_tiles and _new_bbox
        scene = context.scene
        addon = getattr(scene, "blosm", None)
        
        # Get stored tiles again to update them at the end
        stored_tiles = _tile_set_from_prop(scene.get("blosm_import_tiles"))
        ...
        d_south = min(t[0] for t in self._delta_tiles)
        d_west = min(t[1] for t in self._delta_tiles)
        d_north = max(t[2] for t in self._delta_tiles)
        d_east = max(t[3] for t in self._delta_tiles)
        
        # Enforce water handling via water_manager (disable standard water)
        include_water_standard = False
        should_process_water = bool(getattr(addon, "route_import_water", True))

        fetcher = OverpassFetcher(
            user_agent=BLOSM_OT_FetchRouteMap._resolved_user_agent,
            include_roads=True,
            include_buildings=True,
            include_water=include_water_standard,
        ...
        try:
            ...
            try:
                ...
                addon.highways = True
                addon.water = include_water_standard
        ...
        # Reapply key pipeline steps so extended geometry matches main import
        
        # 1. Regenerate Water (if enabled) using the NEW expanded bbox
        if should_process_water:
            try:
                from . import water_manager
                # water_manager.process handles its own fetching and clears previous results,
                # so passing the new full bbox correctly regenerates the complete water system.
                water_manager.process(context, bounds=self._new_bbox)
            except Exception as water_exc:
                print(f"[BLOSM] WARN extend: water processing failed: {water_exc}")

        # 2. Re-apply Building Nodes
        try:
            bldg_summary = route_buildings.apply_building_nodes(context)
            if bldg_summary:
        ...
        scene["blosm_import_bbox"] = list(self._new_bbox)
        new_tiles_set = set(_tile_bbox(*self._new_bbox))
        all_tiles = stored_tiles.union(new_tiles_set)
        scene["blosm_import_tiles"] = [list(t) for t in all_tiles]
        _record_extend_history(scene, self._new_bbox, self._delta_tiles)
        self.report({'INFO'}, f"Extended city by {self._expand_m:.0f} m; fetched {len(self._delta_tiles)} new tile(s).")
        
        # Cleanup
        self._delta_tiles = None
        self._new_bbox = None
        self._dialog_stats = None
